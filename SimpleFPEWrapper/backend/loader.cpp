...
namespace SFPEW::Utils::BackendLoader {
    ...
    bool AcquireGLESFunctions(External::GLESFunctionsTable& funcs, External::EGL::eglGetProcAddress_PTR procAddress) {
        if (!procAddress) {
            return false;
        }
...
#define INIT_GLES_FUNC(name)                                                                                           \
    do {                                                                                                               \
        funcs.name = (External::GLES::name##_PTR)procAddress(#name);                                                   \
    } while (0);
...
#define INIT_EGL_FUNC(name)                                                                                            \
    ...
    bool FillInGLESCapabilities(External::GLESCapabilities& caps, const External::GLESFunctionsTable& glesFuncs) {
        ...
        glesFuncs.glGetIntegerv(GL_MAJOR_VERSION, (GLint*)caps.GLESVersion[0]);
        glesFuncs.glGetIntegerv(GL_MINOR_VERSION, (GLint*)caps.GLESVersion[1]);

        caps.GLESVersionString = std::string((char*)glesFuncs.glGetString(GL_VERSION));
        caps.GLESRendererString = std::string((char*)gpuName);
        caps.GLESVendorString = std::string((char*)vendorName);
        caps.GLESShadingLanguageVersionString = std::string((char*)glesFuncs.glGetString(GL_SHADING_LANGUAGE_VERSION));
